...
public final class CmdLog extends HostLogs {
    ...
    private static Unit[] getUnits() {
        return new Unit[]{new Unit("s", "s", "second", "seconds"), new Unit("m", "m", "minute", "minutes"),
                new Unit("h", "h", "hour", "hours"), new Unit("d", "d", "day", "days")};
    }
    ...
    protected JComponent[] getAdditionalComponents() {
        ...
        logTimeInMinutes = widgetFactory.createInstance(Widget.Type.TEXTFIELDWITHUNIT, DEFAULT_TIME, Widget.NO_ITEMS, getUnits(),
                Widget.NO_REGEXP, 150, Widget.NO_ABBRV, new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL),
                                   Widget.NO_BUTTON);
