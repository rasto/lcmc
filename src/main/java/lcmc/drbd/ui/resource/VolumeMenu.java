...
public class VolumeMenu {
    ...
    public List<UpdatableItem> getPulldownMenu(final VolumeInfo volumeInfo) {
        ...
        connectMenu.addAction(text -> {
            ...
            if (connectMenu.getText()
                    .equals(Tools.getString("ClusterBrowser.Drbd.ResourceConnect") + ' ' + getResourceInfo().getName())) {
        ...
        final ButtonCallback connectItemCallback = getBrowser().new DRBDMenuItemCallback(null).addAction(dcHost -> {
            final BlockDevInfo sourceBDI = getBrowser().getDrbdGraph().getSource(volumeInfo);
            final BlockDevInfo destBDI = getBrowser().getDrbdGraph().getDest(volumeInfo);
        ...
        final MyMenuItem resumeSync = menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Drbd.ResourceResumeSync"), null,
                Tools.getString("ClusterBrowser.Drbd.ResourceResumeSync.ToolTip"),

                Tools.getString("ClusterBrowser.Drbd.ResourcePauseSync"), null,
                Tools.getString("ClusterBrowser.Drbd.ResourcePauseSync.ToolTip"), new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                new AccessMode(AccessMode.OP, AccessMode.NORMAL)).predicate(volumeInfo::isPausedSync).enablePredicate(() -> {
            if (!volumeInfo.isSyncing()) {
                return "it is not syncing";
            }
            return null;
        });
        resumeSync.addAction(text -> {
            final BlockDevInfo sourceBDI = getBrowser().getDrbdGraph().getSource(volumeInfo);
            final BlockDevInfo destBDI = getBrowser().getDrbdGraph().getDest(volumeInfo);
        ...
        /* resolve split-brain */
        final UpdatableItem splitBrainMenu =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Drbd.ResolveSplitBrain"), null,
                        ...
                        .addAction(text -> volumeInfo.resolveSplitBrain());
        items.add(splitBrainMenu);

        /* start online verification */
        final UpdatableItem verifyMenu = menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Drbd.Verify"), null,
                Tools.getString("ClusterBrowser.Drbd.Verify.ToolTip"), new AccessMode(AccessMode.OP, AccessMode.NORMAL),
            ...
            return null;
        }).addAction(text -> volumeInfo.verify(runMode));
        items.add(verifyMenu);
        /* remove resource */
        final UpdatableItem removeResMenu =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Drbd.RemoveEdge"), ClusterBrowser.REMOVE_ICON,
        ...
        final UpdatableItem viewLogMenu =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Drbd.ViewLogs"), VolumeInfo.LOGFILE_ICON, null,
