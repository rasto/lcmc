...
public class ServiceMenu {
    ...
    public List<UpdatableItem> getPulldownMenu(final ServiceInfo serviceInfo) {
        ...
        final ComponentWithTest startMenuItem =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.StartResource"), ServiceInfo.START_ICON,
        ...
        final ButtonCallback startItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                dcHost -> serviceInfo.startResource(dcHost, Application.RunMode.TEST));
        ...
        final ComponentWithTest stopMenuItem =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.StopResource"), ServiceInfo.STOP_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL)).enablePredicate(() -> {
                    if (serviceInfo.getBrowser().crmStatusFailed()) {
                        return ClusterBrowser.UNKNOWN_CLUSTER_STATUS_STRING;
        ...
        final ButtonCallback stopItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                dcHost -> serviceInfo.stopResource(dcHost, Application.RunMode.TEST));
        ...
        final ComponentWithTest upMenuItem =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.UpResource"), ServiceInfo.GROUP_UP_ICON,
                                ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                                new AccessMode(AccessMode.OP, AccessMode.NORMAL))
                        .visiblePredicate(() -> serviceInfo.getGroupInfo() != null)
                        .enablePredicate(() -> {
        ...
        final ButtonCallback upItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                dcHost -> serviceInfo.upResource(dcHost, Application.RunMode.TEST));
        ...
        final ComponentWithTest downMenuItem =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.DownResource"), ServiceInfo.GROUP_DOWN_ICON,
                                ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                                new AccessMode(AccessMode.OP, AccessMode.NORMAL))
                        .visiblePredicate(() -> serviceInfo.getGroupInfo() != null)
                        .enablePredicate(() -> {
        ...
        final ButtonCallback downItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                dcHost -> serviceInfo.downResource(dcHost, Application.RunMode.TEST));
        serviceInfo.addMouseOverListener(downMenuItem, downItemCallback);
        items.add((UpdatableItem) downMenuItem);

        /* clean up resource */
        final UpdatableItem cleanupMenuItem = menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.CleanUpFailedResource"),
                        ServiceInfo.SERVICE_RUNNING_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,

                        Tools.getString("ClusterBrowser.Hb.CleanUpResource"), ServiceInfo.SERVICE_RUNNING_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL))

                .predicate(() -> serviceInfo.getService().isAvailable() && serviceInfo.isOneFailed(runMode)).enablePredicate(() -> {
                    if (serviceInfo.getBrowser().crmStatusFailed()) {
                        return ClusterBrowser.UNKNOWN_CLUSTER_STATUS_STRING;
        ...
        final ComponentWithTest manageMenuItem =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.ManageResource"), ServiceInfo.MANAGE_BY_CRM_ICON,
                                ClusterBrowser.STARTING_PTEST_TOOLTIP,

                                Tools.getString("ClusterBrowser.Hb.UnmanageResource"), ServiceInfo.UNMANAGE_BY_CRM_ICON,
                                ClusterBrowser.STARTING_PTEST_TOOLTIP,

                                new AccessMode(AccessMode.OP, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL))
                        .predicate(() -> !serviceInfo.isManaged(runMode))
                        .enablePredicate(() -> {
                            if (serviceInfo.getBrowser().crmStatusFailed()) {
                                return ClusterBrowser.UNKNOWN_CLUSTER_STATUS_STRING;
                            } else {
                        ...
                        .addAction(text -> {
                            serviceInfo.hidePopup();
                            serviceInfo.setManaged(text.equals(Tools.getString("ClusterBrowser.Hb.ManageResource")),
                                    serviceInfo.getBrowser().getDCHost(), runMode);
                        });
        final ButtonCallback manageItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                dcHost -> serviceInfo.setManaged(!serviceInfo.isManaged(Application.RunMode.TEST), dcHost,
        ...
        if (ci == null) {
            ...
            final ComponentWithTest removeMenuItem =
                    menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.RemoveService"), ClusterBrowser.REMOVE_ICON,
                        ...
                        val groupResources = cs.getGroupResources(serviceInfo.getGroupInfo().getHeartbeatId(runMode), runMode);
            ...
            final ButtonCallback removeItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null) {
                @Override
                public boolean isEnabled() {
                    return super.isEnabled() && !serviceInfo.getService().isNew();
                }
            }.addAction(dcHost -> serviceInfo.removeMyselfNoConfirm(dcHost, Application.RunMode.TEST));
        ...
        final UpdatableItem viewLogMenu =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.ViewServiceLog"), ServiceInfo.LOGFILE_ICON, null,
                        ...
                        .addAction(text -> {
                            ...
                            serviceLogs.init(serviceInfo.getBrowser().getCluster(), serviceInfo.getNameForLog(),
                                    serviceInfo.getService().getCrmId());
                            serviceLogs.showDialog();
                        });
        items.add(viewLogMenu);
        /* more migrate options */
        final MyMenu migrateSubmenu = menuFactory.createMenu(Tools.getString("ClusterBrowser.MigrateSubmenu"),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL))
                .enablePredicate(() -> {
        ...
        /* config files */
        final MyMenu filesSubmenu = menuFactory.createMenu(Tools.getString("ClusterBrowser.FilesSubmenu"),
                        new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL), new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL))
                .enablePredicate(() -> {
                    return null; //TODO: enable only if it has items
                });
        filesSubmenu.onUpdate(() -> {
    ...
    private MyMenu getAddServiceMenuItem(final ServiceInfo serviceInfo,
                                         ...
                                         final String name) {
        final MyMenu serviceMenu = menuFactory.createMenu(name, new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL),
                new AccessMode(AccessMode.OP, AccessMode.NORMAL)).enablePredicate(() -> {
            if (serviceInfo.getBrowser().crmStatusFailed()) {
                return ClusterBrowser.UNKNOWN_CLUSTER_STATUS_STRING;
            } else if (serviceInfo.getService().isRemoved()) {
        ...
        serviceMenu.onUpdate(() -> {
            ...
            for (final String cl : ClusterBrowser.CRM_CLASSES) {
                ...
                if (ResourceAgent.LSB_CLASS_NAME.equals(cl) && !serviceInfo.getAddServiceList(ResourceAgent.SERVICE_CLASS_NAME)
                        .isEmpty()) {
                    mode = AccessMode.ADVANCED;
                }
                ...
                final boolean ret = drbdGui.getScrollingMenu(ClusterBrowser.getClassMenuName(cl), colOrdPanel, classItem, dlm,
                        new MyList<>(dlm, serviceMenu.getBackground()), serviceInfo, popups, null);
                if (!ret) {
    ...
    protected void addMigrateMenuItems(final ServiceInfo serviceInfo, final List<UpdatableItem> items) {
        ...
        for (final Host host : serviceInfo.getBrowser().getClusterHosts()) {
            ...
            final MyMenuItem migrateFromMenuItem =
                    menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.MigrateFromResource") + ' ' + hostName,
                            ...
                            .visiblePredicate(() -> {
                                ...
                                return !serviceInfo.getBrowser().crmStatusFailed() && serviceInfo.getService().isAvailable()
                                       && runningOnNode && host.isCrmStatusOk();
                            })
                            .addAction(text -> {
                                serviceInfo.hidePopup();
                                serviceInfo.migrateFromResource(serviceInfo.getBrowser().getDCHost(), hostName, runMode);
                            });
            final ButtonCallback migrateItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                    dcHost -> serviceInfo.migrateFromResource(dcHost, hostName, Application.RunMode.TEST));
        ...
        final ComponentWithTest unmigrateMenuItem =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.UnmigrateResource"), ServiceInfo.UNMIGRATE_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL)).visiblePredicate(() -> {
                    return !serviceInfo.getBrowser().crmStatusFailed() && serviceInfo.getService().isAvailable() && (
                            serviceInfo.getMigratedTo(runMode) != null || serviceInfo.getMigratedFrom(runMode) != null);
                }).addAction(text -> {
                    serviceInfo.hidePopup();
                    serviceInfo.unmigrateResource(serviceInfo.getBrowser().getDCHost(), runMode);
                });
        final ButtonCallback unmigrateItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                dcHost -> serviceInfo.unmigrateResource(dcHost, Application.RunMode.TEST)
    ...
    protected void addMoreMigrateMenuItems(final ServiceInfo serviceInfo, final MyMenu submenu) {
        ...
        for (final Host host : serviceInfo.getBrowser().getClusterHosts()) {
            ...
            final MyMenuItem migrateMenuItem =
                    menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.MigrateResource") + ' ' + hostName,
                                    ServiceInfo.MIGRATE_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
                            ...
                            .visiblePredicate(() -> {
                                ...
                                return !hostName.equalsIgnoreCase(runningOnNode);
                            })
                            .addAction(text -> {
                                serviceInfo.hidePopup();
                                serviceInfo.migrateResource(hostName, serviceInfo.getBrowser().getDCHost(), runMode);
                            });
            final ButtonCallback migrateItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                    dcHost -> serviceInfo.migrateResource(hostName, dcHost, Application.RunMode.TEST));
        ...
        for (final Host host : serviceInfo.getBrowser().getClusterHosts()) {
            ...
            final MyMenuItem forceMigrateMenuItem =
                    menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.ForceMigrateResource") + ' ' + hostName,
                                    ServiceInfo.MIGRATE_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
                            ...
                            .visiblePredicate(() -> {
                                ...
                                final String runningOnNode = runningOnNodes.get(0).toLowerCase(Locale.US);
                                return !serviceInfo.getBrowser().crmStatusFailed() && serviceInfo.getService().isAvailable()
                                       && !hostName.equalsIgnoreCase(runningOnNode) && host.isCrmStatusOk();
                            })
                            .addAction(text -> {
                                serviceInfo.hidePopup();
                                serviceInfo.forceMigrateResource(hostName, serviceInfo.getBrowser().getDCHost(), runMode);
                            });
            final ButtonCallback forceMigrateItemCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                    dcHost -> serviceInfo.forceMigrateResource(hostName, dcHost, Application.RunMode.TEST));
    ...
    private List<String> getConfigFiles(final ServiceInfo serviceInfo) {
        ...
        final String raName = Objects.requireNonNullElse(cs, serviceInfo).getResourceAgent().getRAString();
        final Host[] hosts = serviceInfo.getBrowser().getCluster().getHostsArray();
        final List<String> cfs = new ArrayList<>(hosts[0].getHostParser().getDistStrings(raName + ".files"));
        final Collection<String> params = new ArrayList<>(hosts[0].getHostParser().getDistStrings(raName + ".params"));
    ...
    protected void addFilesMenuItems(final ServiceInfo serviceInfo, final MyMenu submenu) {
        ...
        for (final String configFile : configFiles) {
            final MyMenuItem fileItem =
                    menuFactory.createMenuItem(configFile, null, null, new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL),
                            new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL)).addAction(text -> {
    ...
    protected void addExistingServiceMenuItem(final ServiceInfo serviceInfo,
                                              ...
                                              final Application.RunMode runMode) {
        final MyMenuItem existingServiceMenu =
                menuFactory.createMenuItem(name, null, null, new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL));
        existingServiceMenu.addAction(text -> {
            final Thread thread = new Thread(() -> {
                ...
                serviceInfo.addServicePanel(otherService, null, colocationWi.isSelected(), orderWi.isSelected(), true,
                        serviceInfo.getBrowser().getDCHost(), runMode);
                swingUtils.invokeLater(existingServiceMenu::repaint);
            });
            thread.start();
        });
        dlm.addElement(existingServiceMenu);
        final ButtonCallback mmiCallback = serviceInfo.getBrowser().new ClMenuItemCallback(null).addAction(
                dcHost -> serviceInfo.addServicePanel(otherService, null, colocationWi.isSelected(), orderWi.isSelected(), true,
    ...
    private MyMenu getExistingServiceMenuItem(final ServiceInfo serviceInfo,
        ...
        serviceMenu.onUpdate(() -> {
            ...
            final MyListModel<MyMenuItem> dlm = new MyListModel<>();
            final Map<MyMenuItem, ButtonCallback> callbackHash = new HashMap<>();
            final MyList<MyMenuItem> list = new MyList<>(dlm, serviceMenu.getBackground());

            final List<JDialog> popups = new ArrayList<>();
            for (final ServiceInfo otherService : serviceInfo.getBrowser().getExistingServiceList(serviceInfo)) {
                ...
                addExistingServiceMenuItem(serviceInfo, otherService.toString(), otherService, dlm, callbackHash, list,
                ...
                if (otherService.getResourceAgent() != null && otherService.getResourceAgent().isGroup()) {
                    ...
                    groupMenu.addExistingGroupServiceMenuItems(serviceInfo, otherService, dlm, callbackHash, list, colocationWi,
                            orderWi, popups, runMode);
                }
            }
            final JPanel colOrdPanel = new JPanel(new FlowLayout(FlowLayout.LEADING, 0, 0));
            colOrdPanel.setBackground(ClusterBrowser.STATUS_BACKGROUND);
            ...
            colOrdPanel.add(orderWi);
            final boolean ret =
                    drbdGui.getScrollingMenu(name, colOrdPanel, serviceMenu, dlm, list, serviceInfo, popups, callbackHash);
            if (!ret) {
                serviceMenu.setEnabled(false);
            }
            serviceMenu.updateMenuComponents();
    ...
    private void addDrbdLinbitMenu(final ServiceInfo serviceInfo,
                                   ...
                                   final ResourceAgent fsService,
                                   final Application.RunMode runMode) {
        final MyMenuItem ldMenuItem = menuFactory.createMenuItem(Tools.getString("ClusterBrowser.linbitDrbdMenuName"), null, null,
                new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL));
        ldMenuItem.addAction(text -> {
            serviceInfo.hidePopup();
            ...
            final FilesystemRaInfo fsi =
                    (FilesystemRaInfo) serviceInfo.addServicePanel(fsService, ldMenuItem.getPos(), true, /* colocation */
                            true, /* order */
                            true, false, runMode);
    ...
    private void addDrbddiskMenu(final ServiceInfo serviceInfo,
                                 ...
                                 final ResourceAgent fsService,
                                 final Application.RunMode runMode) {
        final MyMenuItem ddMenuItem = menuFactory.createMenuItem(Tools.getString("ClusterBrowser.DrbddiskMenuName"), null, null,
                new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL));
        ddMenuItem.addAction(text -> {
            serviceInfo.hidePopup();
            final FilesystemRaInfo fsi =
                    (FilesystemRaInfo) serviceInfo.addServicePanel(fsService, ddMenuItem.getPos(), true, /* colocation */
                            true, /* order */
                            true, false, runMode);
    ...
    private void addIpMenu(final ServiceInfo serviceInfo,
        ...
        final MyMenuItem ipMenuItem = menuFactory.createMenuItem(ipService.getPullDownMenuName(), null, null,
                new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL));
        ipMenuItem.addAction(text -> {
            serviceInfo.hidePopup();
            serviceInfo.addServicePanel(ipService, ipMenuItem.getPos(), true, /* colocation */
                    true, /* order */
                    true, false, runMode);
            serviceInfo.getBrowser().getCrmGraph().repaint();
        });
    ...
    private void addFilesystemMenu(final ServiceInfo serviceInfo,
                                   ...
                                   final ResourceAgent fsService,
                                   final Application.RunMode runMode) {
        final MyMenuItem fsMenuItem = menuFactory.createMenuItem(fsService.getPullDownMenuName(), null, null,
                new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL));
        fsMenuItem.addAction(text -> {
            serviceInfo.hidePopup();
            serviceInfo.addServicePanel(fsService, fsMenuItem.getPos(), true, /* colocation */
                    true, /* order */
                    true, false, runMode);
            serviceInfo.getBrowser().getCrmGraph().repaint();
        });
    ...
    private void addResourceAgentMenu(final ServiceInfo serviceInfo,
                                      ...
                                      final JCheckBox orderWi,
                                      final Application.RunMode runMode) {
        final MyMenuItem resourceAgentMenu = menuFactory.createMenuItem(ra.getPullDownMenuName(), null, null,
                new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL));
        resourceAgentMenu.addAction(text -> {
            serviceInfo.hidePopup();
            swingUtils.invokeLater(() -> {
                for (final JDialog otherP : popups) {
                    otherP.dispose();
            ...
            serviceInfo.addServicePanel(ra, resourceAgentMenu.getPos(), colocationWi.isSelected(), orderWi.isSelected(), true,
                    false, runMode);
            serviceInfo.getBrowser().getCrmGraph().repaint();
        });
    ...
    protected void addDependencyMenuItems(final ServiceInfo serviceInfo,
        ...
        final MyMenuItem addGroupMenuItem =
                menuFactory.createMenuItem(Tools.getString("ClusterBrowser.Hb.AddDependentGroup"), null, null,
                        new AccessMode(AccessMode.ADMIN, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL));
        addGroupMenuItem.enablePredicate(() -> {
            if (serviceInfo.getBrowser().crmStatusFailed()) {
                return ClusterBrowser.UNKNOWN_CLUSTER_STATUS_STRING;
            } else if (serviceInfo.getService().isRemoved()) {
                return ServiceInfo.IS_BEING_REMOVED_STRING;
            ...
            serviceInfo.hidePopup();
            final CrmXml crmXML = serviceInfo.getBrowser().getCrmXml();
            serviceInfo.addServicePanel(crmXML.getGroupResourceAgent(), addGroupMenuItem.getPos(), false, /* colocation only */
                    false, /* order only */
                    true, false, runMode);
            serviceInfo.getBrowser().getCrmGraph().repaint();
        });
        ...
        final MyMenu addServiceMenuItem =
                getAddServiceMenuItem(serviceInfo, runMode, Tools.getString("ClusterBrowser.Hb.AddDependency"));
        items.add(addServiceMenuItem);

        /* add existing service dependency*/
        final MyMenu existingServiceMenuItem =
                getExistingServiceMenuItem(serviceInfo, Tools.getString("ClusterBrowser.Hb.AddStartBefore"), enableForNew, runMode);
