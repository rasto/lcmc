...
public class PcmkMultiSelectionMenu {
    ...
    public List<UpdatableItem> getPulldownMenu(final PcmkMultiSelectionInfo pcmkMultiSelectionInfo) {
        ...
        final Collection<ServiceInfo> selectedServiceInfos = new ArrayList<>();
        final List<HostInfo> selectedHostInfos = new ArrayList<>();
    ...
    private void createSelectedHostsPopup(final List<HostInfo> selectedHostInfos,
        ...
        final MyMenuItem standbyItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StandByOn"), HostInfo.HOST_STANDBY_ICON,
        ...
        final ButtonCallback standbyItemCallback =
                getBrowser().new ClMenuItemCallback(getBrowser().getDCHost()).addAction(dcHost -> {
                    for (final HostInfo hi : selectedHostInfos) {
                        if (!hi.isStandby(Application.RunMode.LIVE)) {
        ...
        final MyMenuItem onlineItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StandByOff"), HostInfo.HOST_STANDBY_OFF_ICON,
                                ClusterBrowser.STARTING_PTEST_TOOLTIP,
        ...
        final MyMenuItem stopCorosyncItem = menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StopCorosync"),
                        HostInfo.HOST_STOP_COMM_LAYER_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,

                        Tools.getString("PcmkMultiSelectionInfo.StopOpenais"), HostInfo.HOST_STOP_COMM_LAYER_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP,
        ...
        /* Stop heartbeat. */
        final MyMenuItem stopHeartbeatItem = menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StopHeartbeat"),
                        HostInfo.HOST_STOP_COMM_LAYER_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
        ...
        /* Start corosync. */
        final MyMenuItem startCorosyncItem = menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StartCorosync"),
                        HostInfo.HOST_START_COMM_LAYER_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
                ...
                .visiblePredicate(() -> {
                    for (final HostInfo hi : selectedHostInfos) {
                        ...
                        if (h.isCorosyncInstalled() && h.hasCorosyncInitScript() && h.corosyncOrOpenaisConfigExists()
                            && !h.isCorosyncRunning() && !h.isOpenaisRunning() && !h.isHeartbeatRunning() && !h.isHeartbeatInRc()) {
                            return true;
                        }
                    }
                    return false;
        ...
        /* Start openais. */
        final MyMenuItem startOpenaisItem = menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StartOpenais"),
                        HostInfo.HOST_START_COMM_LAYER_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
                ...
                .visiblePredicate(() -> {
                    for (final HostInfo hi : selectedHostInfos) {
                        ...
                        if (hostParser.hasOpenaisInitScript() && hostParser.corosyncOrOpenaisConfigExists()
                            && !hostParser.isCorosyncRunning() && !hostParser.isOpenaisRunning() && !hostParser.isHeartbeatRunning()
        ...
        /* Start heartbeat. */
        final MyMenuItem startHeartbeatItem = menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StartHeartbeat"),
                        HostInfo.HOST_START_COMM_LAYER_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
                ...
                .visiblePredicate(() -> {
                    for (final HostInfo hi : selectedHostInfos) {
                        val hostParser = hi.getHost().getHostParser();
                        if (hostParser.hasHeartbeatInitScript() && hostParser.heartbeatConfigExists()
        ...
        /* Start pacemaker. */
        final MyMenuItem startPcmkItem = menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StartPacemaker"),
                        HostInfo.HOST_START_COMM_LAYER_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
                ...
                .visiblePredicate(() -> {
                    for (final HostInfo hi : selectedHostInfos) {
                        val hostParser = hi.getHost().getHostParser();
                        if (!hostParser.isPcmkStartedByCorosync() && !hostParser.isPacemakerRunning() && (
                                hostParser.isCorosyncRunning() || hostParser.isOpenaisRunning())
                            && !hostParser.isHeartbeatRunning()) {
                            return true;
                        }
        ...
        final UpdatableItem changeHostColorItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.ChangeHostColor"), null, "",
    ...
    private void createSelectedServicesPopup(final Iterable<ServiceInfo> selectedServiceInfos,
        ...
        final ComponentWithTest startMenuItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StartSelectedResources"), ServiceInfo.START_ICON,
        ...
        final ButtonCallback startItemCallback = getBrowser().new ClMenuItemCallback(null).addAction(dcHost -> {
            for (final ServiceInfo si : selectedServiceInfos) {
                if (si.isConstraintPlaceholder() || si.getService().isNew() || si.getService().isOrphaned()) {
                    continue;
                }
        ...
        final ComponentWithTest stopMenuItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.StopSelectedResources"), ServiceInfo.STOP_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL)).enablePredicate(() -> {
                    if (getBrowser().crmStatusFailed()) {
                        return ClusterBrowser.UNKNOWN_CLUSTER_STATUS_STRING;
        ...
        final ButtonCallback stopItemCallback = getBrowser().new ClMenuItemCallback(null).addAction(dcHost -> {
            for (final ServiceInfo si : selectedServiceInfos) {
                if (si.isConstraintPlaceholder() || si.getService().isNew() || si.getService().isOrphaned()) {
                    continue;
                }
        ...
        final UpdatableItem cleanupMenuItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.CleanUpFailedResource"),
                        ServiceInfo.SERVICE_RUNNING_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,

                        Tools.getString("PcmkMultiSelectionInfo.CleanUpResource"), ServiceInfo.SERVICE_RUNNING_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL)).predicate(() -> {
                    for (final ServiceInfo si : selectedServiceInfos) {
        ...
        final ComponentWithTest manageMenuItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.ManageResource"), ServiceInfo.MANAGE_BY_CRM_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
        ...
        final ButtonCallback manageItemCallback = getBrowser().new ClMenuItemCallback(null).addAction(dcHost -> {
            for (final ServiceInfo si : selectedServiceInfos) {
                if (si.isConstraintPlaceholder() || si.getService().isNew() || si.getService().isOrphaned()) {
                    continue;
                }
        ...
        final ComponentWithTest unmanageMenuItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.UnmanageResource"),
                                ServiceInfo.UNMANAGE_BY_CRM_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
        ...
        final ButtonCallback unmanageItemCallback = getBrowser().new ClMenuItemCallback(null).addAction(dcHost -> {
            for (final ServiceInfo si : selectedServiceInfos) {
                if (si.isConstraintPlaceholder() || si.getService().isNew() || si.getService().isOrphaned()) {
                    continue;
                }
        ...
        for (final Host host : getBrowser().getClusterHosts()) {
            ...
            final MyMenuItem migrateFromMenuItem =
                    menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.MigrateFromResource") + ' ' + hostName,
                                    ServiceInfo.MIGRATE_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,

                                    Tools.getString("PcmkMultiSelectionInfo.MigrateFromResource") + ' ' + hostName + " (offline)",
                                    ServiceInfo.MIGRATE_ICON, ClusterBrowser.STARTING_PTEST_TOOLTIP,
                                    new AccessMode(AccessMode.OP, AccessMode.NORMAL), new AccessMode(AccessMode.OP, AccessMode.NORMAL))
            ...
            final ButtonCallback migrateItemCallback = getBrowser().new ClMenuItemCallback(null).addAction(dcHost -> {
                for (final ServiceInfo si : selectedServiceInfos) {
                    if (si.isConstraintPlaceholder() || si.getService().isNew() || si.getService().isOrphaned()) {
                        continue;
                    }
        ...
        final ComponentWithTest unmigrateMenuItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.UnmigrateResource"), ServiceInfo.UNMIGRATE_ICON,
                        ClusterBrowser.STARTING_PTEST_TOOLTIP, new AccessMode(AccessMode.OP, AccessMode.NORMAL),
                        new AccessMode(AccessMode.OP, AccessMode.NORMAL)).visiblePredicate(() -> {
                    for (final ServiceInfo si : selectedServiceInfos) {
                        if (si.isConstraintPlaceholder() || si.getService().isNew() || si.getService().isOrphaned()) {
                        ...
                        if (!getBrowser().crmStatusFailed() && si.getService().isAvailable() && (
        ...
        final ButtonCallback unmigrateItemCallback = getBrowser().new ClMenuItemCallback(null).addAction(dcHost -> {
            for (final ServiceInfo si : selectedServiceInfos) {
                if (si.isConstraintPlaceholder() || si.getService().isNew() || si.getService().isOrphaned()) {
                    continue;
                }
        ...
        final ComponentWithTest removeMenuItem =
                menuFactory.createMenuItem(Tools.getString("PcmkMultiSelectionInfo.RemoveService"), ClusterBrowser.REMOVE_ICON,
                    ...
                    for (final ServiceInfo si : selectedServiceInfos) {
                        ...
                        val groupResources =
                                cs.getGroupResources(gi.getHeartbeatId(Application.RunMode.LIVE), Application.RunMode.LIVE);


                        if (groupResources.isEmpty() || groupResources.get().size() <= 1) {
                    ...
                    pcmkMultiSelectionInfo.hidePopup();
                    if (!application.confirmDialog(Tools.getString("PcmkMultiSelectionInfo.confirmRemove.Title"),
        ...
        removeItemCallback.addAction(dcHost -> {
            for (final ServiceInfo si : selectedServiceInfos) {
